...
/// Static section label with baseline divider (non-collapsible shelves like Active).
pub fn sidebar_section_header(
    ...
    h_flex()
        .id(format!(
            ...
            label.split(' ').next().unwrap_or("section")
        ))
        .w_full()
        .min_w_0()
...
pub fn sidebar_shelf_header(
    ...
    let chevron_rotation = Radians(std::f32::consts::PI * progress);
